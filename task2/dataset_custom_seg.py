...
class CustomDataset(Dataset):
    def __init__(self, train_dir, target_dir, transform=None):
        ...
        #print(np.array(Image.open(os.path.join(train_dir, self.images[1])).convert('L')))
        self.crop_ind = crop_index_bw(np.array(Image.open(os.path.join(train_dir, self.images[1])).convert('L')))
        #print(self.crop_ind)
    ...
    def __getitem__(self, idx):
        ...
        while not self.images[idx].endswith(valid_extensions):
            idx += 1
        
        train_img = crop_by_index(np.array(Image.open(os.path.join(self.train_dir, self.images[idx])).convert('L')),self.crop_ind)/255.0
        target_img = crop_by_index(np.array(Image.open(os.path.join(self.target_dir, self.images[idx])).convert('L')),self.crop_ind)/255.0
