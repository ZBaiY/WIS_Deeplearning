...
class DenoiseAutoencoder(nn.Module):
    def __init__(self, device='mps'):
        ...
        self.encoder = nn.Sequential(OrderedDict([
            ('enc_conv1', nn.Conv2d(1, 32, kernel_size=3, stride=2, padding=1)),  # Larger kernel size
            ('enc_bn1', nn.BatchNorm2d(32)),
            ('enc_relu1', nn.LeakyReLU()),
            ('enc_conv2', nn.Conv2d(32, 64, kernel_size=3, stride=2, padding=1)),  # Larger kernel size
            ...
            ('enc_conv4', nn.Conv2d(128, 128, kernel_size=3, stride=2, padding=1)),  # Additional layer
            ('enc_bn4', nn.BatchNorm2d(128)),
            ('enc_relu4', nn.LeakyReLU()),
        ]))
        # Decoder
        self.decoder = nn.Sequential(OrderedDict([
            ('dec_conv1', nn.ConvTranspose2d(128, 128, kernel_size=3, stride=2, padding=1, output_padding=1)),  # Adjusted for additional layer
            ...
            ('dec_conv3', nn.ConvTranspose2d(64, 32, kernel_size=3, stride=2, padding=1, output_padding=1)),  # Larger kernel size
            ('dec_bn3', nn.BatchNorm2d(32)),
            ('dec_relu3', nn.LeakyReLU()),
            ('dec_conv4', nn.ConvTranspose2d(32, 1, kernel_size=3, stride=2, padding=1, output_padding=1)),  # Larger kernel size
