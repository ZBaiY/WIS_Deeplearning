...
class CustomDataset(Dataset):
    def __init__(self, image_paths):
        ...
        #self.noisy_images = [thicken_lines(crop_by_index(np.array(Image.open(path).convert("L")),crop_ind),kernel_size=(2,2),iterations=15) for path in image_paths]  # Convert noisy images to grayscale
        #self.noisy_images = [invert_image(img) for img in self.noisy_images]  # Convert noisy images to grayscale
        self.noisy_images = [thicken_lines(to_bw(crop_by_index(np.array(Image.open(path.replace('01.png', '02.png')).convert("L")),crop_ind)),kernel_size=(2,2),iterations=15) for path in image_paths]  # Convert clean images to grayscale
